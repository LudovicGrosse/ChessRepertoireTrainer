 ...
 * If valid, attaches the decoded user payload to the request object.
 */
...
/**
 * POST /api/register
 * Registers a new user with a hashed password and generates a verification token.
 * Sends a verification email containing a link with the generated token.
 */
  ...
  try {
    ...
    if (err.code === "23505") {
      ...
      res
        .status(500)
        .json({
          error: "Erreur serveur: " + (err.message || "Détails inconnus"),
        });
    }
  }
});
 ...
 * Validates the email verification token and marks the user's account as verified.
 */
...
/**
 * POST /api/login
 * Authenticates a user using either their username or email, checks if their account
 * is verified, and returns a signed JWT token if credentials are valid.
 */
 ...
 * Generates a reset token and sends an email with a password reset link to the user.
 * The link contains the token which is valid for a limited time (1 hour).
 */
 ...
 * Verifies the reset token, updates the user's password hash in the database,
 * and nullifies the token to prevent reuse.
 */
...
/**
 * POST /api/history
 * Records a new history entry for a user's study session.
 * Also synchronizes the repertoire title for all past entries of the same study.
 */
...
/**
 * GET /api/history
 * Retrieves the full study history for the authenticated user, ordered by date descending.
 */
 ...
 * Updates the title of an existing repertoire for all associated history entries.
 * Uses pattern matching to handle both short and full study URLs.
 */
app.put(
  "/api/history/repertoire/title",
  authenticateToken,
  async (req, res) => {
    try {
 ...
 * Deletes all history entries associated with a specific study ID and color.
 */
 ...
 * Updates the title of a specific chapter within a repertoire in the user's history.
 */

...
describe("Auth Endpoints", () => {
  ...
  describe("POST /api/register", () => {
    it("should register a user and return 201", async () => {
      ...
      expect(res.body).toHaveProperty("message", "Compte créé ! Veuillez vérifier votre boîte mail.");
  ...
  describe("POST /api/login", () => {
    it("should login successfully and return token", async () => {
      ...
      db.query.mockResolvedValueOnce({
        rows: [{ id: 1, username: "testuser", email: "test@example.com", password_hash: hashedPassword, is_verified: 1 }],
      });
    ...
    it("should return 403 if email not verified", async () => {
      ...
      expect(res.body).toHaveProperty("error", "Veuillez vérifier votre email avant de vous connecter.");
    });
  });
});

...
- **Déploiement Automatique :** Le site est déployé automatiquement sur Render (via un Webhook caché) _uniquement_ si l'étape de test réussit (`success()`). Render est configuré avec l'Auto-Deploy désactivé pour laisser GitHub Actions prendre la décision.

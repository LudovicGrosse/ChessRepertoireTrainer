...
const updateStatsUI = () => {
    ...
    document.getElementById('statVars').textContent = `${state.learnedVariations}/${state.totalVariations}`;
    document.getElementById('statErrors').textContent = state.errorCount;

    let progress = state.totalMoves > 0 ? Math.round((state.learnedMoves / state.totalMoves) * 100) : 0;
...
const updateAnalysisLink = () => {
    ...
    if (studyUrl) {
        ...
        document.getElementById('analysisLink').href = `${studyUrl}?color=${state.playerColor}#${moveNumber}`;
    } else {
        document.getElementById('analysisLink').href = `https://lichess.org/analysis/standard/${state.game.fen().replace(/ /g, '_')}?color=${state.playerColor}`;
    }
};

const renderPgnHtml = () => {
    if (state.currentPath.length <= 1) {
        document.getElementById('activeLineDisplay').innerHTML = "<span style='color: var(--text-muted);'>Position initiale</span>";
    ...
    if (active) {
        display.scrollTop = active.offsetTop - display.offsetTop - (display.clientHeight / 2) + (active.clientHeight / 2);
    }

    const node = state.currentPath[state.viewIndex];
    document.getElementById('commentBox').innerHTML = node?.comment ? `<strong>Notes :</strong> ${node.comment}` : `<em>Aucun commentaire.</em>`;
};
...
const navigateView = (step) => {
    ...
    state.cg.set({ fen: state.game.fen(), lastMove: state.viewIndex > 0 ? [state.currentPath[state.viewIndex].from, state.currentPath[state.viewIndex].to] : null, drawable: { autoShapes: shapes } });
    updateAnalysisLink();

    if (state.viewIndex === state.currentPath.length - 1 && !state.currentPath[state.viewIndex].isCompleted && state.game.turn() === state.playerColor[0]) {
        preparePlayerTurn();
    } else {
...
const preparePlayerTurn = () => {
    ...
    state.cg.set({ turnColor: state.playerColor, movable: { color: state.playerColor, free: false, showDests: true, dests, events: { after: onUserMove } } });
...
const onUserMove = (orig, dest) => {
    ...
    if (!move || !child) {
        ...
        showToast(state.trainingMode === 'revision' ? "Coup incorrect !" : "Suivez la flèche !", "error", 1500);
        setTimeout(() => {
            state.cg.set({ fen: state.game.fen() });
            preparePlayerTurn();
        }, 600);
    ...
    state.cg.set({ fen: state.game.fen(), lastMove: [orig, dest], movable: { dests: new Map() }, drawable: { autoShapes: [] } });
...
const handleEnd = () => {
    if (state.rootNode.isCompleted) {
        ...
        if (state.authToken && state.trainingMode === 'revision') {
            saveHistory({ repertoire_title: state.currentRepertoire.title, chapter_title: state.currentRepertoire.chapters[state.currentChapterIndex].title, moves_learned: state.learnedMoves, total_moves: state.totalMoves, errors: state.errorCount, is_revision: true });
        }
        ...
        btn.textContent = state.trainingMode === 'decouverte' ? "Passer en mode Révision" : "Refaire ce chapitre";
        btn.style.width = "100%";
...
export const initTraining = () => {
    document.getElementById('startBtn').onclick = () => {
        ...
        if (state.authToken) {
            saveHistory({ repertoire_title: state.currentRepertoire.title, chapter_title: chap.title, moves_learned: 0, total_moves: state.totalMoves, errors: 0, is_revision: false });
        }

        document.getElementById('trainingHeader').innerHTML = `<div style="color: var(--primary);">${state.currentRepertoire.title}</div><div style="font-size: 0.9rem; color: var(--text-muted); font-weight: 500; margin-top: 4px;">${chap.title.replace(state.currentRepertoire.title + ": ", "")}</div>`;
        document.getElementById('trainingActions').classList.add('hidden');

        document.getElementById('errorStatRow').parentElement.style.gridTemplateColumns = state.trainingMode === 'decouverte' ? '1fr 1fr' : '1fr 1fr 1fr';
        document.getElementById('errorStatRow').classList.toggle('hidden', state.trainingMode === 'decouverte');
        document.getElementById('hintBtn').classList.toggle('hidden', state.trainingMode === 'decouverte');
        document.getElementById('analysisLink').classList.toggle('hidden', state.trainingMode === 'revision');
        ...
        setTimeout(() => {
            ...
            showToast(state.trainingMode === 'decouverte' ? "Mode Découverte activé" : "Mode Révision activé", "info");
            startNextVariation();
        }, 50);
    };
    ...
    document.getElementById('hintBtn').onclick = () => {
        ...
        const next = state.currentNode.children.find(c => !c.isCompleted);
        if (next) {
            state.cg.set({ drawable: { autoShapes: [{ orig: next.from, dest: next.to, brush: 'hint' }] } });
